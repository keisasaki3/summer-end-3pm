...
MP3のエンコーダ遅延・末尾/先頭の静音区間によるループ切れを避けるため、マップ環境音は同一音源の2トラックを交互に使う。HTML5 Audio では同一キーを同時再生するために複数の `HTMLAudioElement` が必要なので、各マップ音源はロード時に `instances: 2` を確保する。現行実装では終端5秒前から次トラックを開始し、4秒間クロスフェードする。通常の `loop:true` だけには依存しない。
